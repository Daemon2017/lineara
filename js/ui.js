function renderResults(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        html += '  <span style="font-size: 12px; color: #718096; display: block; margin-bottom: 6px;">Найденные когнаты (Евразийский праязык):</span>';
        var cognatesList = [];
        try {
        ...
        if (cognatesList && cognatesList.length > 0) {
            ...
            html += '        <tr>';
            html += '          <th style="padding: 8px 12px; font-weight: 600;">Реконструкция</th>';
            html += '          <th style="padding: 8px 12px; font-weight: 600;">Значение (Meaning)</th>';
            html += '        </tr>';
            html += '      </thead>';
            html += '      <tbody>';
            ...
            html += '    </table>';
            html += '  </div>';
            html += '  <div style="font-size: 11px; color: #94a3b8; text-align: right; margin-top: 4px;">Всего совпадений: ' + cognatesList.length + '</div>';
        } else {
